/* TODO
 ...
 +-----------+---------------------------------+
| type       | field_prop_type_allowance_value |
+-----------+---------------------------------+
|       No  |                           0.000 |
|       Fi |                           0.030 |
|       Fo |                           0.015 |
...
const handicapFields = {
  ...
  dec_feet: [
    "biggest_down_wind_sail_foo",
    "biggest_down_wind_sail_lea",
    "biggest_down_wind_sail_luf",
    "biggest_jib_foot",
    "biggest_jib_leach",
    "biggest_jib_luff",
    "biggest_staysail_foot",
    "biggest_staysail_leach",
    "biggest_staysail_luff",
    "fore_triangle_base",
    ...
    "mizzen_luff",
    "mizzen_topsail_luff",
    "mizzen_topsail_perpendicul",
    "topsail_luff",
    "topsail_perpendicular"
  ],
...
const buildBoatNumberQuery = (id, fieldMap) => {
  const q = `SELECT n.nid as entity_id, n.status as published, ${buildFields(
    fieldMap
  )}
    FROM node AS n
    ${buildJoins(fieldMap)}
...
const getTargetField = async (db, field, id) => {
  const [
    ...
    `SELECT IFNULL(field_${field}_value,'') as name FROM field_data_field_${field} WHERE entity_id=?`,
    [id]
  );
  if (l.length > 0) {
    return l[0].name;
...
const numBoats = async db => {
  const [c] = await db.query(
    "SELECT count(*) as num FROM node WHERE type='boat'"
  );
  return c[0].num;
};
...
const numPublishedBoats = async db => {
  const [c] = await db.query(
    "SELECT count(*) as num FROM node WHERE type='boat' AND status=1"
  );
  return c[0].num;
};
...
const numUnpublishedBoats = async db => {
  const [c] = await db.query(
    "SELECT count(*) as num FROM node WHERE type='boat' AND status=0"
  );
  return c[0].num;
};
    ...
    if (fieldMappings.value[key]) {
      ...
      switch (key) {
        ...
        case "maxYear":
          ...
          break;
        case "for_sale":
          wheres += ` AND f_${field}.field_${field}_value IS ${
            filters.for_sale ? "NOT " : ""
          }NULL`;
...
const getTargetIdsForType = async (db, type) => {
  return (
    await db.query("SELECT title as name, nid as id FROM node WHERE type=?", [
      type
    ])
  )[0];
};
...
const getTaxonomy = async (db, type) => {
  const [
    ...
    "SELECT d.name FROM taxonomy_vocabulary v JOIN taxonomy_term_data d on d.vid = v.vid where v.machine_name=?",
    [type]
  );
...
const getBoatsFields = () => {
  let fields =
    "n.nid as entity_id, n.changed as updated, n.status as published,";
...
const getBoats = async (db, filters) => {
  ...
  const [limits, hasNextPage, hasPreviousPage] = getBoatsLimits(
    filters,
    totalCount
  );
